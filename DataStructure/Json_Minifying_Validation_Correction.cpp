...
void mutual (string ID1, string ID2, string inputXML, string mutualOutput)
{
    ...
    ifstream fin(inputXML, ios::in);
    ofstream fout (mutualOutput, ios::out);
    user person1;
    user person2;
    ...
    for(int i =0; i < allUsers.listOfUsers.size();i++){
        ...
        if (ID1==allUsers.listOfUsers[i].id){
            person1 = allUsers.listOfUsers[i];

        }
        if (ID2==allUsers.listOfUsers[i].id){

             person2 = allUsers.listOfUsers[i];
    ...
    for (int i = 0; i < person1._followers.listOfFollowers.size(); i++)
    {
        stringstream ssId(person1._followers.listOfFollowers[i].id);
        int id = 0;
        ssId >> id ;
        idOfUser[id] = true ;
    }
    for (int i = 0; i < person2._followers.listOfFollowers.size(); i++)
    {
        stringstream ssId2(person2._followers.listOfFollowers[i].id);
        int id = 0;
        ssId2 >> id;
        if (idOfUser[id] == true && person2._followers.listOfFollowers[i].id!= person1.id && person2._followers.listOfFollowers[i].id!= person2.id)
        {
            fout << "\nuser of id " << person2._followers.listOfFollowers[i].id << " is mutual between them";
        }
    }
    fin.close();
    fout.close();
...
void suggest(string ID, string inputXML, string suggestOutput)
{
    ...
    user person;
    ifstream fin(inputXML, ios::in);
    ofstream fout (suggestOutput, ios::out);
    string suggested = "ID of The suggested followers:\n";

    //allUsers.buildFollowersList();
    for(int i =0; i < allUsers.listOfUsers.size();i++){

        if (ID==allUsers.listOfUsers[i].id){
          person = allUsers.listOfUsers[i];
          break;}
    }
...
{
    ifstream fin(inputXML, ios::in);
    ofstream fout(maxFollowers, ios::out);

    user maxf;
    if(fin.is_open()&& fout.is_open()){
    users allUsers = readAndCorrect(inputXML);
    user maxf = allUsers.userWithMaxFollowers();
    fout<<"User ID:" << maxf.id << endl << "User Name:"<< maxf.name;
    }
...
{
    ifstream fin(inputXML, ios::in);
    ofstream fout(MostConnected, ios::out);
...
//   for (int i = 0; i < 50; i++) counterOfMostRepeated[i] = 0;
    ...
    fout << "most connected user is of id " << idOfMostConnected << endl;
    fin.close();
    fout.close();
}
...
void minify(string inputXML, string afterMinifying)
{
    ...
    ifstream fin(inputXML, ios::in);
    ofstream fout(afterMinifying, ios::out);
    string _read = "", _line = "";
    ...
    while (getline(fin,_line))
    {
        for (int i = 0; i < _line.size(); i++)
        {
            ...
            _read += _line[i];
            //fout.write(_read.c_str(), sizeof(char)*_read.size());
        }
    }
    fout<<_read;
    fin.close();
